...
geom_segment(data=grid_data, aes(x = end, y = 2.0, xend = start, yend = 2.0), colour = "grey", alpha=1, size=0.3 , inherit.aes = FALSE ) + 
...
geom_text(data=base_data, aes(x = title, y = -0.5, label=group), hjust=c(0,0,0,1,1,1), colour = "black", alpha=0.8, size=4, fontface="bold", inherit.aes = FALSE)
